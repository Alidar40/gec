...
def masked_cross_entropy(logits, target, length):
    ...
    log_probs_flat = F.log_softmax(logits_flat, dim=0)
...
def training(encoder, decoder, encoder_optim, decoder_optim, train_iter, test_iter, opts, load_checkpoint, checkpoint, elmo, experiment_name, wandb=None):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        # validation
        validation(test_iter, encoder, decoder, elmo, opts, wandb, epoch, global_step)
...
def validation(valid_iter, encoder, decoder, elmo, opts, wandb, epoch, global_step):
    total_loss = 0
    total_corrects = 0
    total_words = 0
    encoder.eval()
